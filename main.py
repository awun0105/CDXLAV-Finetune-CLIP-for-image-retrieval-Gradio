            ...
            with gr.Row():
                ...
                with gr.Column():
                    top_k_slider = gr.Slider(
                        label="Top K", minimum=1, maximum=50, step=1, value=5, info="Top K closest results to the query")
                    use_cluster_search = gr.Checkbox(
                        label="Use cluster search", value=False, info="Faster embedding search using clusters, may be less accurate")
                    search_by_text_btn = gr.Button("Search by text")
                    search_by_image_btn = gr.Button("Search by image")

            path = gr.Textbox(
                label="Path", info="Path with images to scan", value=path)
            scan_dir_btn = gr.Button("Scan directory", variant="primary")

        gallery = gr.Gallery(label="Gallery", show_label=False,
                             columns=3, rows=2, height="auto", preview=False)
